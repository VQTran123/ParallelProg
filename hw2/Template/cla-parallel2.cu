...
__device__
int* grab_slice1(int* input, int starti, int length)
{
...
void compute_group_gp()
{
  ...
  for(int j = index; j < ngroups; j += stride){
    ...
    int* ggj_group = grab_slice1(gi,jstart,block_size);
    int* gpj_group = grab_slice1(pi,jstart,block_size);
    ...
    gpj[j] = mult;
      // free from grab_slice1 allocation
    free(ggj_group);
    free(gpj_group);
  }
...
void compute_section_gp()
{
  ...
  for(int k = index; k < nsections; k += stride){
    ...
    int* sgk_group = grab_slice1(ggj,kstart,block_size);
    int* spk_group = grab_slice1(gpj,kstart,block_size);
...
void compute_super_section_gp()
{
  ...
  for(int l = index; l < nsupersections; l += stride){
      ...
      int* ssgl_group = grab_slice1(sgk,lstart,block_size);
      int* sspl_group = grab_slice1(spk,lstart,block_size);
...
void compute_super_super_section_gp()
{
  int index = threadIdx.x + blockIdx.x * blockDim.x;
  int stride = blockDim.x * gridDim.x;
  for(int m = index; m < nsupersections; m += stride){
      int mstart = m*block_size;
      int* sssgm_group = grab_slice1(ssgl,mstart,block_size);
      int* ssspm_group = grab_slice1(sspl,mstart,block_size);
